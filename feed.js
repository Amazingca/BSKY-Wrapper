...
export async function modifyFlow() {
  ...
  try {
    ...
    for (var i = 0; i < userCache.length; i++) {
      ...
      //grab user obj from com.atproto.repo.describe
      var tempReq = await fetch(`https://bsky.social/xrpc/com.atproto.repo.describe?user=${userCache[i]}`).then(r => r);

      
      if (tempReq.status != 200) {
        
        continue;
      }
      
      var userDataObj = JSON.parse(await tempReq.text());
      ...
      const userTemp = JSON.parse(await fetch(`https://bsky.social/xrpc/com.atproto.repo.listRecords?user=${userCache[i]}&collection=app.bsky.actor.profile`).then(r => r.text()));
      
      var username;
      var userClasses = "";

      if ((userTemp.records.length > 0) && (userTemp.records.length != 0)) {

        username = userTemp.records[0].value.displayName;
      } else {

        userClasses = "hidden";
      }

      //if user has datastore, grab it from com.atproto.repo.listRecords
      const userPostStore = JSON.parse(await fetch(`https://bsky.social/xrpc/com.atproto.repo.listRecords?user=${userCache[i]}&collection=app.bsky.feed.post`).then(r => r.text()));

      //parse most recent at top by iso timestamp somehow quickly, or just take most recent post by that user and include that
      const userMostRecentPost = userPostStore.records[0];
      
      var posttext;
      
      if (userMostRecentPost.value.embed != undefined) {
        
        if (userMostRecentPost.value.text != "") {
          
          posttext = userMostRecentPost.value.text.replaceAll("\n", "<br>") + "<br><br>" + embeds(userMostRecentPost.value.embed);
        } else {
          
          posttext = embeds(userMostRecentPost.value.embed);
        }
      } else {
      ...
      const postStruct = 
        `<div class="feed-container">
            ...
            <h3 style="color: #555;">@<a href="${document.location.origin + document.location.pathname}?username=${userDataObj.handle}" title="Go to User Profile">${userDataObj.handle}</a></h3>
            <br>
            <div>${posttext}</div>
            <br>
            <flex>
              <div style="color: #333;">${new Date(userMostRecentPost.value.createdAt)}</div>
              <div style="color: #333;"><a href="${document.location.origin + document.location.pathname}?username=${userDataObj.handle}&postid=${userMostRecentPost.uri.split("/")[userMostRecentPost.uri.split("/").length - 1]}">id#${userMostRecentPost.uri.split("/")[userMostRecentPost.uri.split("/").length - 1]}</a></div>
            </flex>
          </div>`;
      
      sortPosts(Date.parse(userMostRecentPost.value.createdAt), postStruct)
      
      //recentCache = recentCache + postStruct
