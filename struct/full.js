...
export async function postModalBuild() {
  ...
  document.getElementById("flex-post-area").innerHTML =
    `<spacer>${userProfileImage}
      ...
      <div id="text-amount-indicator">250</div>
      <div class="post-options"></div>
  ...
  document.getElementById("post-text").addEventListener("input", function() {
    ...
    document.getElementById("text-amount-indicator").style.setProperty("--deg", (document.getElementById("post-text").value.split("").length * (360 / 256)) + "deg");
    document.getElementById("text-amount-indicator").innerHTML = (256 - document.getElementById("post-text").value.split("").length);
  });
}
